...
void renderChunk(GLFWwindow *window, struct Chunk *chunk, unsigned int shaderProgram, unsigned int VAO){
    mat4 model;
    for(int x = 0; x < 16; x++){
        for(int y = 0; y < 256; y++){
            for(int z = 0; z < 16; z++){
                ...
                glm_translate(model, chunk->blocks[x][y][z].blockPosition);
...
int main(){
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        renderChunk(window, chunk, shaderProgram, VAO);
